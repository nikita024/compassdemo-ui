...
const SetupEstimator = ({ isSetupEstimatorOpen, setIsSetupEstimatorOpen }) => {
  ...
  useEffect(() => {
    const allFieldsFilled = forms.every(form => form.field !== '' && form.condition !== '');
    setIsSaveEnabled(allFieldsFilled);
  }, [forms]);

  const handleAddForm = () => {
    const newFieldNumber = forms.length + 1;
    setForms([...forms, { id: Date.now(), field: '', condition: '', display: false }]);
  };
      ...
      <div className="footer">
        ...
        <div className="right-buttons">
          <Button 
            variant='outlined' 
            className="button" 
            onClick={() => setIsSetupEstimatorOpen(false)}
          >
            Cancel
          </Button>
          <Button 
